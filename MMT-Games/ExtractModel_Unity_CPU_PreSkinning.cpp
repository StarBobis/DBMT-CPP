...
void Functions_Unity_CPU_PreSkinning::ExtractModel() {
    ...
    for (const auto& pair : this->DrawIB_ExtractConfig_Map) {
        ...
        uint64_t TotalUniqueVertexCount = 0;
        for (const auto& pair : firstIndexFileNameMap) {
            ...
            LOG.Info(L"UniqueVertexCount(Before):" + std::to_wstring(ibBufFile.UniqueVertexCount));

            if (G.DontSplitModelByMatchFirstIndex) {
                LOG.Info(L"开启特殊设置，不分割模型，所以仅输出一次");
                ibBufFile.SaveToFile_UINT32(OutputIBBufFilePath, 0);
                TotalUniqueVertexCount += ibBufFile.UniqueVertexCount;
                ...
                LOG.Info(L"UniqueVertexCount:" + std::to_wstring(ibBufFile.UniqueVertexCount));
                TotalUniqueVertexCount += ibBufFile.UniqueVertexCount;
        ...
        LOG.Info(L"TotalUniqueVertexCount:" + std::to_wstring(TotalUniqueVertexCount));
