...
std::wstring Functions_HSR::AutoGameType(std::wstring DrawIB) {
	FrameAnalysisData FAData(G.WorkFolder, DrawIB);

	std::map<std::wstring, uint64_t> PointlistIndexVertexCountMap = FAData.Get_PointlistIndex_VertexCount_Map(VertexCountType::BufFileTotal);
	std::map<std::wstring, uint64_t> TrianglelistIndexVertexCountMap = FAData.Get_TrianglelistIndex_VertexCount_Map(VertexCountType::TxtFileShow, DrawIB);
