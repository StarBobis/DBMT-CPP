...
void Functions_SnB::ExtractFromBuffer_VS_UE4(std::wstring DrawIB, std::wstring GameType) {
    ...
    DrawIBConfig extractConfig = this->DrawIB_ExtractConfig_Map[DrawIB];

    D3D11GameType d3d11GameType = d3d11GameTypeLv2.GameTypeName_D3d11GameType_Map[MMTString::ToByteString(GameType)];
    LOG.Info("GameType: " + d3d11GameType.GameType);

    std::wstring OutputDrawIBFolder = G.Path_OutputFolder + DrawIB + L"\\";
    ...
    for (std::wstring filename : FrameAnalyseFileNameList) {
        ...
        uint64_t POSITION_FileSize = MMTFile::GetFileSize(G.WorkFolder + PositionExtractFileName);
...
void Functions_SnB::ExtractModel() {
    ...
    for (const auto& pair : this->DrawIB_ExtractConfig_Map) {
        ...
        //鸣潮中的虚假IB检测 8d45cfee
        if (DrawIB == L"8d45cfee" && (G.CurrentGameName == L"WW" || G.CurrentGameName == L"WW1.1" || G.CurrentGameName == L"WWMI")) {
        ...
        if (extractConfig.GameType == L"Auto") {
            ...
            for (const auto& pair : d3d11GameTypeLv2.GameTypeName_D3d11GameType_Map) {
                ...
                for (const auto& pair : d3d11GameType.CategorySlotMap) {
                    ...
                    std::wstring CategoryExtractFileName = CategoryExtractFileNameList[0];
                    LOG.Info(L"CategoryExtractFileName" + CategoryExtractFileName);
                    uint64_t CategoryRealStride = MMTFile::GetFileSize(G.WorkFolder + CategoryExtractFileName) / MatchNumber;
            ...
            if (MatchedGameTypeList.size() == 0) {
                ...
                for (std::string matchedGameType : MatchedGameTypeList) {
                    D3D11GameType matchedTestD3D11GameType = d3d11GameTypeLv2.GameTypeName_D3d11GameType_Map[matchedGameType];
