...
void Functions_SnB::GenerateMod() {
    ...
    std::wstring ModOutputFolder = G.Path_OutputFolder + timeStr;
    ...
    for (const auto& pair : this->DrawIB_ExtractConfig_Map) {
        ...
        D3D11GameType d3d11GameType = d3d11GameTypeLv2.GameTypeName_D3d11GameType_Map[extractConfig.WorkGameType];

        std::wstring timeStr = MMTString::GetFormattedDateTimeForFilename().substr(0, 10);
        std::wstring BufferReadFolder = G.Path_OutputFolder + DrawIB + L"\\";
        std::wstring BufferOutputFolder = G.Path_OutputFolder + timeStr + L"\\" + DrawIB + L"\\";
        ...
        std::unordered_map<std::string, uint64_t> CategoryStrideMap = d3d11GameType.getCategoryStrideMap(extractConfig.TmpElementList);
        ...
        for (int i = 0; i < extractConfig.PartNameList.size(); i++) {
                ...
                for (size_t i = 0; i < CategoryList.size(); ++i) {
                    ...
                    if (category == "Normal") {
                        for (int index = 0; index < tmpCategoryData.size(); index = index + 8) {
                            ...
                            //1.获取NORMAL和TANGENT值
                            std::byte TangentValueX = tmpCategoryData[index + 0];
                            std::byte TangentValueY = tmpCategoryData[index + 1];
                            std::byte TangentValueZ = tmpCategoryData[index + 2];
                            std::byte TangentValueW = tmpCategoryData[index + 3];

                            std::byte NormalValueX = tmpCategoryData[index + 4];
                            std::byte NormalValueY = tmpCategoryData[index + 5];
                            std::byte NormalValueZ = tmpCategoryData[index + 6];
                            std::byte NormalValueW = tmpCategoryData[index + 7];

                            //TANGENT不变，第四位全设为127
                            tmpCategoryData[index + 0] = TangentValueX;
                            tmpCategoryData[index + 1] = TangentValueY;
                            tmpCategoryData[index + 2] = TangentValueZ;
                            tmpCategoryData[index + 3] = std::byte(127);

                            //NORMAL前三位翻转，第四位设为TANGENT的W分量
                            tmpCategoryData[index + 4] = MMTFormat::ReverseSNORMValueSingle(NormalValueX);
                            tmpCategoryData[index + 5] = MMTFormat::ReverseSNORMValueSingle(NormalValueY);
                            tmpCategoryData[index + 6] = MMTFormat::ReverseSNORMValueSingle(NormalValueZ);
                            tmpCategoryData[index + 7] = TangentValueW;

                            /*if (TangentValueW == std::byte(127)) {
                                TangentValueW = std::byte(-127);
                            }
                            else if (TangentValueW == std::byte(-127)) {
                                TangentValueW = std::byte(127);
                            }*/
                        }
                    ...
                    if (category == "Blend" && d3d11GameType.UE4PatchNullInBlend) {
            ...
            for (std::string categoryName : CategoryList) {
                ...
                uint64_t fileSize = MMTFile::GetFileSize(filePath);
