...
IndexBufferTxtFile::IndexBufferTxtFile(std::wstring fileName,bool readIBData) {
    ...
    std::set<uint64_t> uniqueVertexCountSet;
            ...
            if (!readIBData) {
                ...
                if (splitLine.size() == 3) {
                    for (std::wstring splitLineStr: splitLine) {
                        uint64_t tmpNumber = std::stoi(splitLineStr);
    ...
    this->UniqueVertexCount = (uint64_t) uniqueVertexCountSet.size();

    //LOG.Info(L"IndexBuffer Parse Over");
}


void IndexBufferTxtFile::SaveToFile_UINT32(std::wstring FileWritePath, uint64_t Offset) {
    std::ofstream file(FileWritePath, std::ios::binary);
    for (const auto& data : this->NumberList) {
        uint64_t paddedData = data + Offset;
        file.write(reinterpret_cast<const char*>(&paddedData), sizeof(uint64_t));
    }
    file.close();
}